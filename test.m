...
commands = categorical(["bed","bird","cat","dog","eight","five","four","happy","house","marvin","nine","one","seven","sheila","six","three","tree","two","wow","zero"]);
...
for i = 1:numel(adsTrain.Files)
    ...
    fullPath = fullfile('newcommandsdata', 'TrainData', label, filename);
...
for i = 1:numel(adsValidation.Files)
    ...
    % Generate the filename using the label and index to ensure uniqueness
    filename = sprintf('%s_%s_%d.png', label, fileName, i);
    fullPath = fullfile('newcommandsdata', 'ValData', label, filename);
